<?php 
	Class Transaction extends MY_Controller{
        ...
        function process(){
            ...
            $transaction = $this->transaction_model->get_info($id);
            if(!$transaction){
                $this->session->set_flashdata('message', 'This Exit not transaction have id !');
                redirect(admin_url('transaction'));
            }
            $input = array();
            ...
            $input = array('status' => 1, 'user_checkout' => $admin_info->name,'day_checkout' => date("Y-m-d H:i:s",time()));


            $this->transaction_model->update($id,$input);
            redirect(admin_url('transaction'));
        }
	}