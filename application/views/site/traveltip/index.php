<main role="main">
    ...
    <div class="l-constrained l-section--lg faux faux--primary">
        <div class="l-primary l-padding-td l-primary-wd">
            <?php foreach($traveltip_list as $row) : ?>
                <div class="row">
                        <div class="pod">
                             <a class="box" href="<?php echo base_url('traveltip/view/'.$row->id)?>" title="<?=$row->title?>">
                                 <div class="box__image-wrap medium-4 column">
                                     <img class="box__image" src="<?php echo base_url('/upload/traveltip/'.$row->image)?>" alt="<?=$row->title?>" title="<?=$row->title?>">
                                 </div>
                                 <div class="medium-8 column">
                                     <h1 class="box__heading" style="font-size:20px;"><?=$row->title?></h1>
                                     <?php 
                                          if(strlen($row->info) < 200){
                                              echo '<span>'.$row->info.'</span>';
                                          }else{
                                            echo '<span>'.mb_substr($row->info,0,200,'utf8').'...'.'</span>';
                                          }
                                      ?>
                                 </div>
                             </a>
