<?php 
	$config = array(
		'admin' => array('index', 'add', 'edit', 'delete'),
		'user' => array('index', 'add', 'edit', 'delete'),
		'tour' => array('index', 'add', 'edit', 'delete', 'del_all'),
		'catalog' => array('index', 'add', 'edit', 'delete', 'del_all'),
		'menu' => array('index', 'add', 'edit', 'delete', 'del_all'),
		'transaction' => array('index', 'add', 'edit', 'delete', 'del_all','process'),
		'news' => array('index', 'add', 'edit', 'delete', 'del_all'),
		'slide' => array('index', 'add', 'edit', 'delete', 'del_all'),
		'phone' => array('index', 'add', 'edit', 'delete', 'del_all'),
		'email' => array('index', 'add', 'edit', 'delete', 'del_all'),
	);
	