		...
		function __construct(){
			...
			switch ($controller) {
				...
				default:
					...
					$input['where'] = array('status' => 1);
					$input['limit'] = array(1, 0);
					$email_list = $this->email_model->get_list($input);
					$this->data['email_list'] = $email_list;

					//lay nhieu HotLine.
					
					$input = array();
					$more_phone_list = $this->phone_model->get_list();
					$this->data['more_phone_list'] = $more_phone_list;


					//lay nhieu Email.
					
					$input = array();
					$more_email_list = $this->email_model->get_list();
					$this->data['more_email_list'] = $more_email_list;

					//lay menu top
					$input = array();
					$this->load->model('menu_model');
					$input['where'] = array('status' => 1, 'top'=>0);
					$input['order'] = array('sort_order', 'ASC');
					$menu_list_top = $this->menu_model->get_list($input);
					$this->data['menu_list_top'] = $menu_list_top;
					...
					$input['where'] = array("status" => 1,'top'=>1);
					$input['order'] = array('sort_order', 'ASC');
